...
(async () => {
	...
	let tx = (await program.methods.initializeMint().accountsStrict({
	...
	}).signers([feePayer, mintKeypair]).transaction());
	const sig = await sendAndConfirmTx(tx, connection, wallet, [mintKeypair]);
	console.log("Created SBT token mint and initialized metadata:", { sig, mint: mint.toBase58() });
})();
